...
void Object::CreateAll()
{
    ...
    for (dodBYTE type = 0; type < 18; ++type)
    {
        const dodBYTE startlevel = OMXTAB[type] >> 4;
        dodBYTE count            = OMXTAB[type] & 0x0F;

        dodBYTE lvl = startlevel;
        do
        {
            const int idx = OBIRTH(type, lvl);
            OCBLND[idx].P_OCOWN = 0xFF; // set as owned by creature
            ++lvl;
            if (lvl > 4)
            {
                lvl = startlevel;
...
int Object::OFIND(RowCol rc)
{
    ...
    while ( !(OCBLND[idx].P_OCROW == rc.row &&
              OCBLND[idx].P_OCCOL == rc.col) || OCBLND[idx].P_OCOWN != 0 );
    return idx;
}
...
void Object::OBJNAM(int idx) const
{
    if (idx == -1)
    {
        // return "EMPTY"
...
bool Object::PAROBJ(dodBYTE& objclass, dodBYTE& objtype, bool& is_specific) const
{
    dodBYTE A, B;

    is_specific = false;
    int res = parser.PARSER(GENTAB, A, B, true);
    if (res == 0)
    {
        parser.CMDERR();
        return false;
